...
import { NestedObject } from "@sadoprotocol/ordit-sdk/dist/utils/types";
import {
  SkipStrictSatsCheckOptions,
  UTXOLimited,
} from "@sadoprotocol/ordit-sdk/dist/transactions/types";
import { buildWitnessScript } from "./witness";

bitcoin.initEccLib(ecc);

export class CustomInscriber extends PSBTBuilder {
  ...
  private getMetadata() {
    return this.meta && this.encodeMetadata
      ? encodeObject(this.meta)
      : this.meta;
  }
  ...
  private isBuilt() {
    if (!this.commitAddress || !this.fee) {
      throw new OrditSDKError(
        "Invalid tx! Make sure you generate commit address or recover and finally build"
      );
    }
  }
  ...
  private async preview(
    { activate }: Record<"activate", boolean> = { activate: true }
  ) {
    if (activate) {
      this.previewMode = true;
      this.suitableUnspent = this.recovery
        ? (await this.datasource.getUnspents({ address: this.commitAddress }))
            .spendableUTXOs[0]
        : getDummyP2TRInput();
